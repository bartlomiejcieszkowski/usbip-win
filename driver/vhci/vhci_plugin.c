...
vhci_plugin_dev(ioctl_usbip_vhci_plugin *plugin, pusbip_vhub_dev_t vhub, PFILE_OBJECT fo)
{
	...
	DBGI(DBG_IOCTL, "Plugin vpdo: port: %hhd, vendor:product: %04hx:%04hx", plugin->port, plugin->vendor, plugin->product);
	...
	// Create the vpdo
	DBGI(DBG_PNP, "vhub->NextLowerDriver = 0x%p", vhub->NextLowerDriver);
